...
void ALobbyPlayersGameMode::AddPlayerDataInInstance(const FPlayerNetData& InPlayerData)
{
	if (UUGCGameInstance * MyGameInstance = GetGameInstance<UUGCGameInstance>())
	{		
		UE_LOG(LogTemp, Display, TEXT("[class ALobbyPlayersGameMode::AddPlayerDataInInstance]：[%d]"), InPlayerData.PlayerID);
		for(auto& Tmp : InPlayerData.EquipedWeapons)
			UE_LOG(LogTemp, Display, TEXT("[class ALobbyPlayersGameMode::AddPlayerDataInInstance]：[%d]"), Tmp);
